...
<head>
    ...
    <title>Edit Course - CAPDEVPRO</title>
...
<body>
    ...
    <script>
        ...
        function ensureModuleExam(wrapper, prefill){
                    ...
                    <div class="exam-questions" style="margin-top:10px">
                        ...
                        <div class="exam-q-builder" style="margin-top:10px;border-top:1px dashed #e5e7eb;padding-top:10px">
                            ...
                            <div class="eq-id" style="display:none;margin-top:8px">
                                <label class="q-label" style="margin-bottom:6px">Teacher Notes / Guide (Optional)</label>
                                <textarea class="eq-id-answer" placeholder="Optional notes or expected answer guide for manual checking" style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:8px;resize:vertical;" rows="2"></textarea>
                            </div>
                            <div class="eq-tf" style="display:none;margin-top:8px">
                                <label class="q-label" style="margin-bottom:6px">Correct Answer</label>
            ...
            if(prefill){
                try{
                    const timerMode = prefill.timer_mode || (prefill.timer_minutes > 0 ? 'timed' : 'untimed');
                    ...
                    if(timerMode === 'untimed'){
                        ...
                        if(input) { input.required = true; input.value = prefill.timer_minutes || ''; }
                    }
                    const listEl = host.querySelector('.exam-q-list');
                    listEl.innerHTML = '';
        ...
        function createCourseExam(afterEl, prefill){
            ...
            function syncExamJSON(){
                ...
                const val = JSON.stringify({ title, description, timer_minutes: duration, timer_mode: timerMode, passing_score: passingScore, questions: qs });
            ...
            mods.forEach((mod, mi)=>{
                ...
                if(mod.exam){
                    ...
                    if(host){
                        ...
                        const timerMode = mod.exam.timer_mode || (mod.exam.timer_minutes > 0 ? 'timed' : 'untimed');
                        const modeEl = host.querySelector('.exam-timer-mode');
                        if(modeEl) modeEl.value = timerMode;
                        const wrapperDur = host.querySelector('.exam-duration-wrapper');
                        const inputDur = host.querySelector('.exam-duration');
                        if(timerMode === 'untimed'){
                            if(wrapperDur) wrapperDur.style.display = 'none';
                            if(inputDur) { inputDur.required = false; inputDur.value = ''; }
                        } else {
                            if(wrapperDur) wrapperDur.style.display = 'block';
                            if(inputDur) { inputDur.required = true; inputDur.value = mod.exam.timer_minutes || ''; }
                        }
