...
<body class="{{ request()->boolean('embedded') ? 'embedded-create' : '' }}">
    ...
    <script>
        ...
        function addOptionRow(container, placeholder){
            ...
            // Add "add option" link only once at the end
        ...
        function syncQuestionsJSON(panel){
            ...
            list.forEach(b=>{
                ...
                if(['multiple_choice','checkboxes','dropdown'].includes(type)){
                    ...
                    q.options = opts;
                    q.other = !!(b.querySelector('.q-other') && b.querySelector('.q-other').checked);
        ...
        function ensureDefaultModule(){
            if(window.__defaultModuleInitialized) return;
            const container = document.getElementById('modulesContainer');
            if(container.children.length > 0){ window.__defaultModuleInitialized = true; return; }
            createModule();
            const wrapper = container.children[0];
            const body = wrapper.querySelector('.module-body');
            body.style.display = 'block';
            addTopicInput(body);
            const topicRow = body.querySelector('.topic-row');
            if(topicRow){
                addSubtopicRow(topicRow);
                const panel = topicRow.querySelector('.subtopic-row:last-of-type .fields-panel');
                if(panel){ 
                    addTextField(panel);
                    const last = panel.querySelector('.field-block:last-of-type');
                    if(last){ 
                        setSelectedField(last);
                        if(typeof setActiveAnchor === 'function'){ setActiveAnchor(last); }
                    }
                }
            }
            window.__defaultModuleInitialized = true;
            updateProgress();
        }
        function bindTabs(){
